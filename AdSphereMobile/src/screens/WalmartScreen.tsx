...
const WalmartScreen = () => {
  ...
  const loadWalmartCampaigns = async () => {
    try {
      const data = await apiService.getWalmartCampaigns();
      setCampaigns(data.data);
    } catch (error) {
      console.error('Failed to load Walmart campaigns:', error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleDownload = () => {
    try {
      exportToCSV(campaigns, 'walmart-campaigns');
    } catch (error) {
  ...
  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Loading Walmart campaigns...</Text>
      </View>
    );
  }
  ...
  return (
    ...
    >
      <View style={styles.header}>
        <View>
          <Text style={styles.title}>🏪 Walmart Campaigns</Text>
      ...
      {campaigns.map((campaign) => (
        <View key={campaign.campaignId} style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <Text style={[
              styles.status,
              campaign.status === 'ACTIVE' ? styles.active : styles.paused
            ]}>
              {campaign.status}
            </Text>
          </View>
          ...
          {campaign.metrics.conversions && (
            <Text style={styles.conversions}>Conversions: {campaign.metrics.conversions}</Text>
          )}
...
const styles = StyleSheet.create({
  ...
  downloadButtonText: { color: '#0071CE', fontSize: 12, fontWeight: '600' },
  card: { 
    backgroundColor: '#FFFFFF', 
    margin: 16, 
    padding: 16, 
  ...
  budget: { fontSize: 14, color: '#0071CE', fontWeight: '600', marginBottom: 12 },
  ...
  conversions: { fontSize: 14, color: '#34C759', fontWeight: '600', marginBottom: 8 },
  date: { fontSize: 12, color: '#8E8E93' },
});

export default WalmartScreen;
